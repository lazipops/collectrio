...
if (isset($_SESSION['username'])) { //check if user is logged in
    if ($_POST['username'] != '' && $_POST['password'] != '' && $_POST['email'] != '') { //check if request fields are not empty
    ...
    } elseif ($_POST['username'] != '' && $_POST['password'] != '') { //check if request fields are not empty
        $username = mysqli_real_escape_string($con, $_POST['username']);
        $password =  mysqli_real_escape_string($con, $_POST['password']);
        $id = $_SESSION['id'];
        ...
        $query = "UPDATE users SET email = '$email', passhash = '" . md5($password) . "' WHERE id = '$id'";
        if ($con->query($query) === TRUE) {
            $_SESSION['flash_message'] = 'Your account details have been changed';
            $_SESSION['username'] = $_POST['username'];
            header("Location: profile.php");
